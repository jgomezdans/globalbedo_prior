...
class GlobAlbedoPrior(object):
    ...
    def __init__(
        ...
        no_snow: bool
            A flag indicating whether to choose the snow free or the snow 
            albedo results
        """
        self.tile = tile
        ...
        LOG.info(
            "Found %d(MCD43A1)/%d(MCD43A2) HDF files in %s"
            % (a1, a2, self.data_dir)
        )
    ...
    def get_modis_fnames(self):
        ...
        for doy in range(1, 367, 8):
            pattern = "MCD43A1.A????%03d.%s.006.*.hdf" % (doy, self.tile)
            self.fnames_mcd43a1[doy] = [
                f for f in locate(pattern, root=self.data_dir)
            ]
            pattern = "MCD43A2.A????%03d.%s.006.*.hdf" % (doy, self.tile)
            self.fnames_mcd43a2[doy] = [
                f for f in locate(pattern, root=self.data_dir)
            ]
        ...
        if a1_files != a2_files:
            raise IOError(
                "I didn't find the same number of MCD43A1 and "
                + "MCD43A2 files!"
            )
        if a1_files == 0:
            raise IOError(
                "No files found!!! You sure about the tile and " + "directory?"
            )
    ...
    def _interpret_qa(self, qa_data):
        ...
        weight = np.where(qa_data < 4, MAGIC ** qa_data, 0.0)
        return weight
    ...
    def _interpret_landcover(self, landcover):
        ...
        See https://ladsweb.modaps.eosdis.nasa.gov/filespec/MODIS/6/MCD43A2
        for details
        
        Returns
        A boolean array of suitable/unsuitable landcover type
        """
        return landcover < 7  # ignore deep ocean
        ...
        for band in self.bands:
            for doy in self.fnames_mcd43a1.keys():
                for kernel in ["iso", "volu", "geo"]:
                    ...
                    self.output_ptrs[output_prod] = drv.Create(
                        output_fname,
                        2400,
                        2400,
                        2,
    ...
    def stage1_prior(self):
        ...
        for band in self.bands:
            for doy in range(1, 367, 8):
                ...
                snow_fnames = [
                    'HDF4_EOS:EOS_GRID:"%s":MOD_Grid_BRDF:Snow_BRDF_Albedo'
                    % (f)
                    for f in self.fnames_mcd43a2[doy]
                ]
                land_fnames = [
                    'HDF4_EOS:EOS_GRID:"%s":MOD_Grid_BRDF:BRDF_Albedo_LandWaterType'
                    % (f)
                    for f in self.fnames_mcd43a2[doy]
                ]

                # Previous code appears inefficient, but remember that we
                # process per chunk, so doesn't matter.
                import ipdb
                ...
                for (
                    ...
                    LOG.info(
                        "\t\tRead a %d x %d pixel chunk..."
                        % (ny_valid, nx_valid)
                    )
                    n_years = (
                        len(data_in) // 4
                    )  # First lot will be BRDF parameters, 2nd will be QA,
                    ...
                    for i, kernel in enumerate(["iso", "volu", "geo"]):
                        ...
                        bout.WriteArray(
                            prior_mean[i, :, :].astype(np.float32),
                            xoff=this_X,
                            yoff=this_Y,
                        )
                        bout = self.output_ptrs[output_prod].GetRasterBand(2)
                        bout.WriteArray(
                            prior_var[i, :, :].astype(np.float32),
                            xoff=this_X,
                            yoff=this_Y,
                        )
                output_prod = "%03d_%s_b%d" % (doy, kernel, band)
                self.output_ptrs[output_prod].SetGeoTransform(
                    ds_config["geoT"]
                )
